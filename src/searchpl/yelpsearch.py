...
original author gchin.
Repository : https://github.com/Yelp/yelp-api
"""
import json
import oauth2
import optparse
...
parser.add_option('--path', '--path' , dest = 'path', help='path from host', default='/v2/search')
...
parser.add_option('-x', '--deals', dest='deals', help='Deals filter')
parser.add_option('-m', '--sort', dest='sort', help='Sort')
parser.add_option('-z','--id',dest='id', help='Business yelp id')
host = 'api.yelp.com'
...
def main(args):
    ...
    if not options.location and not options.bounds and not options.point and not options.id:
        parser.error('--location, --bounds, or --point or the business id required required')
    ...
    return request(host, path, url_params, options.consumer_key, options.consumer_secret, options.token, options.token_secret)
...
#iresponse = request(options.host, '/v2/search', url_params, options.consumer_key, options.consumer_secret, options.token, options.token_secret)
